...
READ_OPERATIONS = [
    'find', 'find_one', 'aggregate', 'count', 'count_documents',
    'distinct', 'find_one_and_update', 'find_one_and_replace',
    'find_one_and_delete'
]

WRITE_OPERATIONS = [
    'insert', 'insert_one', 'insert_many', 'update', 'update_one',
    'update_many', 'replace_one', 'delete', 'delete_one', 'delete_many',
    'create_index', 'drop', 'drop_index'
]
...
class DatabaseCouplingAnalyzer:
    ...
    def _categorize_operations(
        ...
        results = {
            ...
            'other_reads': 0,
            'other_writes': 0,
            'operations_by_collection': defaultdict(lambda: {'reads': 0, 'writes': 0}),
            'violations': [],
        }
    ...
    def finalize_results(self) -> dict[str, Any]:
        ...
        for coll_data in self.results['collections_accessed'].values():
            coll_data['accessed_by_modules'] = list(coll_data['accessed_by_modules'])
        ...
        # Keep only top 20
        self.results['top_coupled_files'] = self.results['top_coupled_files'][:20]
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            'violation_count_read': len([v for v in self.results['violations'] if v['type'] == 'read']),
            'violation_count_write': len([v for v in self.results['violations'] if v['type'] == 'write']),
