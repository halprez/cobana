...
class ClassMetricsAnalyzer:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        """
        self.config = config
        self.class_methods_threshold = config.get('thresholds', {}).get('class_methods', 20)
        self.class_wmc_threshold = config.get('thresholds', {}).get('class_wmc', 50)
        self.class_lcom_threshold = config.get('thresholds', {}).get('class_lcom', 2)
    ...
    def _analyze_class(
        ...
        cohesion_level = 'high' if lcom <= 1 else 'moderate' if lcom == 2 else 'low'
        ...
        # Check if god class
        is_god_class = (
            method_count > self.class_methods_threshold or
            wmc > self.class_wmc_threshold or
            lcom > self.class_lcom_threshold
        )
    ...
    def _update_results(self, class_info: dict[str, Any], module_name: str) -> None:
        ...
        if any(
            gc['class'] == class_info['class'] and gc['file'] == class_info['file']
    ...
    def finalize_results(self) -> dict[str, Any]:
        ...
        if self.results['total_classes'] > 0:
            self.results['avg_lcom'] = self.results['lcom_sum'] / self.results['total_classes']
            self.results['avg_wmc'] = self.results['wmc_sum'] / self.results['total_classes']
        ...
        self.results['low_cohesion_classes'].sort(key=lambda x: x['lcom'], reverse=True)
