...
class ComplexityAnalyzer:
    ...
    def analyze_file(self, file_path: Path, module_name: str) -> dict[str, Any]:
        ...
        file_results = self._process_complexity_blocks(
            complexity_blocks, file_path, module_name
        )
    ...
    def _process_complexity_blocks(
        self, blocks: list, file_path: Path, module_name: str
    ) -> dict[str, Any]:
    ...
    def _update_results(
        ...
        module_stats["complexity_sum"] += sum(
            f["complexity"] for f in file_results["functions"]
        )
        ...
        module_stats["high_complexity_count"] += sum(
            1
            for f in file_results["functions"]
            if f["complexity"] > self.threshold
        )
    ...
    def finalize_results(self) -> dict[str, Any]:
        ...
        if self.results["total_functions"] > 0:
            ...
            self.results["avg_complexity"] = (
                total_complexity / self.results["total_functions"]
            )
        ...
        for module_name, module_stats in self.results["by_module"].items():
            if module_stats["total_functions"] > 0:
                module_stats["avg_complexity"] = (
                    module_stats["complexity_sum"]
                    / module_stats["total_functions"]
                )
        ...
        # Add high complexity count to main results
        self.results["high_complexity_count"] = len(
            self.results["high_complexity_functions"]
        )
